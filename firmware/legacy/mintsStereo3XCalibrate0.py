...
class minstThreeFoldCamParametors(object):
    ...
    def __init__(self, filepath,xPoints,yPoints):
        ...
        self.readAll(self.cal_path)




    def readAll(self, cal_path):

        readLeft(self,cal_path)
    ...
    def readLeft(self, cal_path):
        ...
        for i, fname in enumerate(images):
            ...
            if (ret is True)
                ...
                cv2.drawChessboardCorners(img, (self.yPoints, self.xPoints),
                                                  corners, ret)
        ...
        undistLeft = cv2.undistort(self.leftImagesAllOrig[-1], self.MLeft, self.d1, None,self.MLeft)
        ...
        #
        gray = cv2.cvtColor(undistLeft, cv2.COLOR_BGR2GRAY)
        ...
        rect  = [corners_l[-1][0],corners_l[self.yPoints-1][0],corners_l[0][0],corners_l[-self.yPoints][0]]
        pts  = np.array(rectLeft, dtype = "float32")


        camData = {
		"cameraMatrixLeft" :self.MLeft,
        "distortionCoefficientsLeft" :self.d1,
        "cropPointsLeft":   ptsLeft,
	     }
    ...
    #
    #         img_l =resizeImage(self,cv2.imread(images_left[i]),100)
    #         img_r = resizeImage(self,cv2.imread(images_right[i]),100)
    #
    ...
    #
    #     camData = {
	# 	"cameraMatrixLeft" :self.M1,
	# 	"cameraMatrixRight" :self.M2,
    #     "distortionCoefficientsLeft" :self.d1,
    #     "distortionCoefficientsRight" :self.d2,
    #     "cropPointsLeft":   ptsLeft,
    #     "cropPointsRight":  ptsRight
	#      }
    #
    #
    #     with open('stereoDataAug9.pickle', 'wb') as f:
    #         pickle.dump(camData, f)
    #
    #     #
    #     #
...
if __name__ == '__main__':
    ...
    cal_data = minstStereoAlign(parser['filePath'],\
                                parser['xSquares']-1,\
                                parser['ySquares']-1\
                                )
    # # print("--------------------")
